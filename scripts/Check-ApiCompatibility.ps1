...
.DESCRIPTION
    ...
    apicompat message, '#'-prefixed lines are comments).
...
$srcProject = Join-Path $repoRoot 'src' 'Wolfgang.Extensions.IAsyncEnumerable' 'Wolfgang.Extensions.IAsyncEnumerable.csproj'
$packageId = 'Wolfgang.Extensions.IAsyncEnumerable'

[xml]$projectXml = Get-Content $srcProject -Raw
$currentVersionRaw = $projectXml.Project.PropertyGroup.Version | Where-Object { $_ } | Select-Object -First 1
if (-not $currentVersionRaw) {
    Write-Error "Could not read <Version> from $srcProject"
    exit 1
}
...
$downloadDir = Join-Path $repoRoot 'apicompat-previous'
...
foreach ($previousDll in $previousLibDlls) {
    ...
    $currentDll = Join-Path $repoRoot 'src' 'Wolfgang.Extensions.IAsyncEnumerable' 'bin' $Configuration $tfm "$packageId.dll"
    ...
    Write-Host "`n=== Comparing $tfm ===" -ForegroundColor Cyan
    ...
    foreach ($line in $output) {
        ...
        if ($isSuppressed) {
            ...
            $unsuppressedBreaks += "$tfm`: $line"
        }
    }
}

if ($unsuppressedBreaks.Count -gt 0) {
    Write-Host "`n==========================================" -ForegroundColor Red
    Write-Host "❌ UNSUPPRESSED ABI BREAKS DETECTED" -ForegroundColor Red
    Write-Host "==========================================" -ForegroundColor Red
    foreach ($b in $unsuppressedBreaks) {
...
Write-Host "`n✅ No disallowed ABI breaks against $previousVersion." -ForegroundColor Green
exit 0
